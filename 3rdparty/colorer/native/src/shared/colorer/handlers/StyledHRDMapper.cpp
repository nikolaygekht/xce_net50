...
void StyledHRDMapper::loadRegionMappings(InputSource *is)
{
  ...
  for(Node *curel = hbase->getFirstChild(); curel; curel = curel->getNextSibling()){
    if (curel->getNodeType() == Node::ELEMENT_NODE && *curel->getNodeName() == "assign"){
      ...
      int val = 0;
      int fore, back;
      if (UnicodeTools::getNumber(((Element*)curel)->getAttribute(DString("fore")), &val))
        fore = val;
      else
        fore = -1;
      if (UnicodeTools::getNumber(((Element*)curel)->getAttribute(DString("back")), &val))
        back = val;
       else
        back = -1;
      UnicodeTools::getNumber(((Element*)curel)->getAttribute(DString("cfore")), &val);
      int cfore = val;
      UnicodeTools::getNumber(((Element*)curel)->getAttribute(DString("cback")), &val);
      int cback = val;
      int style;
      if (UnicodeTools::getNumber(((Element*)curel)->getAttribute(DString("style")), &val))
        ...
        style = 0;
      RegionDefine *rdef = new StyledRegion(cfore, cback, fore, back, style);
