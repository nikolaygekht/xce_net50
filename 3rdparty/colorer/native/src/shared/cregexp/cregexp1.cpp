...
StackElem1 *RegExpStack1;
int RegExpStack_Size1;
/////////////////////////////////////////////////////////////////////////////
//
SRegInfo1::SRegInfo1()
{
...
CRegExp1::CRegExp1(const String *text)
{
  init();
  if (text) setRE(text);
};
...
EError1 CRegExp1::setRELow(const String &expr)
{
  int len = expr.length();
  ...
  tree_root = new SRegInfo1;
  tree_root->op = ReBrackets;
  tree_root->un.param = new SRegInfo1;
  tree_root->un.param->parent = tree_root;
  tree_root->param0 = cMatch++;

  int endPos;
  EError1 err = setStructs(tree_root->un.param, DString(&expr, start, len), endPos);
...
void CRegExp1::optimize()
{
SRegInfo1 *next = tree_root;
  firstChar = BAD_WCHAR;
...
EError1 CRegExp1::setStructs(SRegInfo1 *&re, const String &expr, int &retPos)
{
SRegInfo1 *next, *temp;
  ...
  for(int i = 0; i < expr.length(); i++){
    ...
    if (expr[i] == '\\'){
      ...
      switch (expr[i+1]){
        ...
        case 'n':
          next->op = ReMetaSymb;
          next->un.metaSymbol = ReCrLf;
          break;
        case 'N':
          next->op = ReMetaSymb;
          next->un.metaSymbol = ReNCrLf;
          break;
        case 'b':
          next->op = ReMetaSymb;
    ...
    if (expr[i] == '('){
      ...
      next->un.param = new SRegInfo1;
      next->un.param->parent = next;
      int endPos;
      EError1 err = setStructs(next->un.param, DString(&expr, i), endPos);
    ...
    if (expr[i] == '['){
      int endPos;
      CharacterClass *cc = CharacterClass::createCharClass1(expr, i, &endPos);
...
//      next->op = (exprn[i] == ReEnumS) ? ReEnum : ReNEnum;
  ...
  for(next = re; next; next = next->next){
    ...
    SRegInfo1 *reword = next;
    SRegInfo1 *reafterword = next;
    SRegInfo1 *resymb;
    int wsize = 0;
...
bool CRegExp1::isWordBoundary(int &toParse)
{
  int before = 0;
  int after  = 0;
...
bool CRegExp1::isNWordBoundary(int &toParse)
{
...
bool CRegExp1::checkMetaSymbol(EMetaSymbols1 symb, int &toParse)
{
const String &pattern = *global_pattern;
  ...
  switch(symb){
    ...
    case ReEoL:
      if (multiLine){
        ...
        if (toParse && toParse < end &&
                         (pattern[toParse] == 0x0A || pattern[toParse] == 0x0B ||
                          pattern[toParse] == 0x0C || pattern[toParse] == 0x0D ||
                          pattern[toParse] == 0x85 ||
                          pattern[toParse] == 0x2028 ||
                          pattern[toParse] == 0x2029)) ok = true;
    ...
    case ReNWordSymb:
      ...
      return true;
    case ReWSpace:
      if (toParse >= end || !Character::isWhitespace1(pattern[toParse])) return false;
      toParse++;
      return true;
    case ReNWSpace:
      if (toParse >= end || Character::isWhitespace2(pattern[toParse])) return false;
      toParse++;
      return true;
    case ReCrLf:
      if (toParse >= end || !Character::isCrLf(pattern[toParse])) return false;
      toParse++;
      return true;
    case ReNCrLf:
      if (toParse >= end || Character::isCrLf(pattern[toParse])) return false;
...
void CRegExp1::check_stack(bool res,SRegInfo1 **re, SRegInfo1 **prev, int *toParse, bool *leftenter, int *action)
{
  if (count_elem==0){
    *action=res;
    return;
  ...
  StackElem1 &ne=RegExpStack1[--count_elem];
  if (res){
...
void CRegExp1::insert_stack(SRegInfo1 **re, SRegInfo1 **prev, int *toParse, bool *leftenter, int ifTrueReturn, int ifFalseReturn, SRegInfo1 **re2, SRegInfo1 **prev2, int toParse2)
{
  if (RegExpStack_Size1==0){
    RegExpStack1 = new StackElem1 [INIT_MEM_SIZE];
    RegExpStack_Size1 = INIT_MEM_SIZE;
  }
  if(RegExpStack_Size1==count_elem){
    RegExpStack_Size1+= MEM_INC;
    StackElem1* s = new StackElem1 [RegExpStack_Size1];
    memcpy(s,RegExpStack1,count_elem*sizeof(StackElem1));
    delete[] RegExpStack1;
    RegExpStack1=s;
  }
  StackElem1 &ne=RegExpStack1[count_elem++];
...
bool CRegExp1::lowParse(SRegInfo1 *re, SRegInfo1 *prev, int toParse)
{
int i, sv, wlen;
bool leftenter = true;
bool br = false;
...
#else
        {
          SMatch1 *mt = namedMatches->getItem(re->namedata);
          if (!mt) {
            check_stack(false,&re,&prev,&toParse,&leftenter,&action);
            continue;
          }
...
inline bool CRegExp1::quickCheck(int toParse)
{
...
inline bool CRegExp1::parseRE(int pos)
{
...
bool CRegExp1::parse(const String *str, int pos, int eol, SMatches1 *mtch
#ifdef NAMED_MATCHES_IN_HASH
, PMatchHash nmtch
#endif
...
bool CRegExp1::parse(const String *str, SMatches1 *mtch
#ifdef NAMED_MATCHES_IN_HASH
,PMatchHash nmtch
#endif
)
...
bool CRegExp1::setRE(const String *re)
{
  error = EError;
#ifdef NAMED_MATCHES_IN_HASH
  PMatchHash oldnamedMatches = namedMatches;
  SMatchHash1 tmpMatchHash;
...
EError1 CRegExp1::getError()
{
  return error;
};

bool CRegExp1::setPositionMoves(bool moves)
{
  positionMoves = moves;
  return true;
};
...
#ifndef NAMED_MATCHES_IN_HASH
int CRegExp1::getBracketNo(const String *brname)
{
  for(int brn = 0; brn < cnMatch; brn++)
    if (brname->equalsIgnoreCase(brnames[brn])) return brn;
  return -1;
};
String *CRegExp1::getBracketName(int no)
{
...
#ifdef COLORERMODE
bool CRegExp1::setBackRE(CRegExp1 *bkre)
{
  this->backRE = bkre;
  return true;
};
bool CRegExp1::setBackTrace(const String *str, SMatches1 *trace)
{
  backTrace = trace;
  backStr = str;
  return true;
};
bool CRegExp1::getBackTrace(const String **str, SMatches1 **trace)
{
  *str = backStr;
  *trace = backTrace;
  return true;
